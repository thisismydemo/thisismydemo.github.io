...
description: Evidence-backed comparison of VMware VCF, Azure Local, Hyper-V on SAN, and Hyper-V on S2D — migration paths, storage flexibility, resilience, and TCO.
...
| **Host maintenance workflows** | How disruptive is patching and hardware work? | Lifecycle Manager via Azure portal. CAU integrated. | Cluster-Aware Updating (CAU). Drain, patch, resume. SCVMM can orchestrate. | Same as SAN—CAU plus optional SCVMM orchestration. | VCF lifecycle manager is more integrated. Hyper-V CAU is entirely functional but less elegant. All platforms achieve rolling maintenance without VM downtime. |
| **Storage migration for running VMs** | Moving workloads without forcing all storage decisions on day one | Limited. S2D-internal storage. FC SAN preview is too early. | **Strongest answer.** Storage Live Migration moves VHDs between CSVs, LUNs, or SMB shares with zero downtime. Array-native tools add further flexibility. | Storage Live Migration within S2D pool. Less flexible than external SAN for cross-platform movement. | **Hyper-V on SAN wins.** Decoupled compute and storage means workloads can land on interim storage and migrate later without platform constraints. |
...
| **Storage expansion** | How painful is adding capacity? | Add nodes (compute + storage together). S2D expansion requires careful planning. | **Independent scaling.** Add storage without compute. Add compute without storage. | Add disks or nodes. Rebalancing required. | **Hyper-V on SAN wins on independent scaling.** HCI models tie compute and storage expansion together. |
...
When Hyper-V is backed by enterprise SAN storage, the DR options aren't just adequate—they're enterprise-grade and multi-layered. Here's the full resilience stack available to Hyper-V + WSFC deployments:
...
With **Azure Local**, the VM lands on S2D storage inside the cluster. The source SAN cannot be reused directly unless it happens to be Dell PowerFlex. The hardware must be from the Azure Local catalog. This narrows the migration path and may force a hardware purchase before migration can begin.
...
| **Hardware requirement** | Any Windows Server HCL hardware | Any Windows Server HCL hardware | Validated nodes only |
...
Once VMs are on Hyper-V, the DR story isn't just adequate—it's multi-layered in a way Azure Local cannot match.
...
These advantages are most pronounced in Windows-heavy environments with existing SAN infrastructure—which describes a large percentage of the organizations currently reassessing their VMware position.
